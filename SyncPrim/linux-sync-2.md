...
which compares the `old` with the value pointed to by `ptr`.  If they equal, it stores the `new` in the memory location which is pointed by the `ptr` and returns the initial value in this memory location.
